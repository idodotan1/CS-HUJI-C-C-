...
private:
    std::string _name;
    rank_map _my_ratings;
    std::shared_ptr<RecommendationSystem> _s_rs;
public:
	/**
	 * Constructor for the class
	 */
	User(const std::string& name,const rank_map& my_ratings,
         std::shared_ptr<RecommendationSystem> s_rs) : _name(name),
         _my_ratings(my_ratings),_s_rs(std::move(s_rs)) {}
	 ...
	 */
	double get_rs_prediction_score_for_movie(const std::string& name, int year,
                                             int k) const;
